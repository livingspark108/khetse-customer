...
class APIHelper {
  ...
  Future<dynamic> addAddress(Address address) async {
    try {
      ...
      print("ADDRESS FORM DATA = ${formData.fields}");
      print("ADDRESS REQUEST = ${global.baseUrl}add_address");
      final _headers = await global.getApiHeaders(true);
  ...
  Future<dynamic> addToCart({int? qty, int? varientId, int? special}) async {
    try {
      ...
      print("REQUEST = ${global.baseUrl}add_to_cart\n${formData.fields}");
  ...
  Future<dynamic> getCategoryProducts(
    ...
    try {
      ...
      print("getCategoryProducts DATA = ${formData.fields}");
      print(
          "URL getCategoryProducts = ${global.baseUrl}cat_product?page=$page");
  ...
  Future<dynamic> getHomeScreenData() async {
    try {
      ...
      var dio = Dio();
      var formData = FormData.fromMap({
        'store_id': global.nearStoreModel?.id ?? 2,
        'user_id': global.currentUser?.id,
      });
  ...
  Future<dynamic> getNearbyStore() async {
    try {
      ...
      dynamic recordList;
      print("LANG = ${global.languageCode}");
      print("FORMDATA = ${formData.fields}");
      print("STORE RESPONSE = ${response.data}");
      if (response.statusCode == 200 && '${response.data['status']}' == '1') {
  ...
  Future<dynamic> getTopSellingProducts(
    ...
    try {
      ...
      print(
          "TOP SELLING PRODUCTS API = ${global.baseUrl}top_selling?page=$page");
      print("API PAYLOAD = ${formData.fields}");
      dynamic recordList;
      if (response.statusCode == 200 && response.data["status"] == '1') {
        recordList = List<Product>.from(
            response.data["data"].map((x) => Product.fromJson(x)));
  ...
  Future<dynamic> membershipStatus() async {
    try {
      ...
      print("Exception - membershipStatus(): " +
          e.toString() +
  ...
  Future<dynamic> rechargeWallet(String rechargeStatus, double amount,
    ...
    try {
      ...
      dynamic recordList;
      if ((response.statusCode == 200 && response.data["status"] == '1')) {
        LocalNotificationService _localNotification =
            LocalNotificationService();
        await _localNotification.init();

        await _localNotification.sendNotification(
  ...
  Future<dynamic> updateProfile(CurrentUser user) async {
    try {
      ...
      final _apiHeaders = await global.getApiHeaders(true);

      print("HEADERS = $_apiHeaders");
  ...
  Future<String?> uploadImageToStorage(XFile image, String? chatId,
  ...
  Future<ReviewModel?> getOrderRatings({
    required int orderId,
  }) async {
    var dio = Dio();

    final url = "${global.baseUrl}get_order_review/$orderId";
  ...
  Future<bool> postOrderRating({
    required int orderId,
    required int vegetable,
    ...
    final url = "${global.baseUrl}submit_order_review";

    final payload = {
      "order_id": orderId,
      "vegetables_fruits_quality": vegetable,
