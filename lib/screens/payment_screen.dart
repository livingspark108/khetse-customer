...
class _PaymentGatewayScreenState extends BaseRouteState {
  ...
  Order? order;
  _PaymentGatewayScreenState(this.screenId, this.totalAmount,
      this.membershipModel, this.order, this.cartController)
      : super();
  ...
  void createOrderId() async {
    ...
    var authn = 'Basic ' +
        base64Encode(utf8.encode(
            '${global.paymentGateway!.razorpay!.razorpayKey}:${global.paymentGateway!.razorpay!.razorpaySecret}'));
  ...
  void payStack(String? key) async {
    try {
      payPlugin
        ...
        print(
            "Exception - internal error - paymentGatewaysScreen.dart - payStatck(): " +
                e.toString());
      });
    } catch (e) {
  ...
  _getAppInfo() async {
    try {
      bool isConnected = await br.checkConnectivity();
      if (isConnected) {
        await apiHelper.getAppInfo(global.currentUser!.id).then((result) async {
          if (result != null) {
            if (result.status == "1") {
  ...
  _chargeCard(Charge charge) async {
    try {
      payPlugin.chargeCard(context, charge: charge).then((value) async {
        ...
        print(
            "Exception - inner error - paymentGatewaysScreen.dart - paystack - _chargeCard(): " +
                e.toString());
      });
    } catch (e) {
