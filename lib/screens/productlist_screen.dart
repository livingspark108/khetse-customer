...
class ProductListScreen extends BaseRoute {
  ...
  ProductListScreen({super.analytics, super.observer, super.routeName = 'ProductListScreen', this.screenId, this.categoryId, this.categoryName});
  ...
  _applyFilters() async {
    try {
      showModalBottomSheet(
        ...
        context: context,
        builder: (context) => Padding(padding: EdgeInsets.only(top: 100), child: FilterScreen(_productFilter, isProductAvailable: _productsList.length > 0 ? true : false)),
      ).then((value) async {
        if (value != null) {
          _isDataLoaded = false;
  ...
  _getProductList() async {
    try {
      ...
      if (isConnected) {
        ...
        _productFilter.maxPriceValue = _productsList.length > 0 ? _productsList[0].maxprice : 0;
      } else {
